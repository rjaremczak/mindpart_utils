...
static enum error_code transfer_fd(int src_fd, int dst_fd, size_t len, struct crc16* crc16) {
    char data;
    for(size_t i = 0; i < len; i++) {
        if(read(src_fd, &data, 1) != 1) return READ_ERROR;
        if(write(dst_fd, &data, 1) != 1) return WRITE_ERROR;
        if(crc16) crc16_update(crc16, data);
        usleep(5000);
    }
...
static enum error_code send_fd(const void* data, size_t len, int dst_fd, struct crc16* crc16) {
    for(size_t i = 0; i < len; i++) {
        if(write(dst_fd, ((const char*)data) + i, 1) != 1) return -2;
        if(crc16) crc16_update(crc16, ((const char*)data)[i]);
        usleep(5000);
    }
    printf("sent: %ld\n", len);
    return NO_ERROR;
...
int main(int argc, char* argv[]) {
    ...
    struct crc16 crc16;
    crc16_init(&crc16);
    struct bin_header header = {.type_id = 0x01, .data_size = src_stat.st_size, .load_address = 0x0400};

    if((error_code = send_fd(&header, sizeof(header), dst_fd, &crc16))) goto error_exit;
    written += sizeof(struct bin_header);

    if((error_code = transfer_fd(src_fd, dst_fd, src_stat.st_size, &crc16)) != 0) goto error_exit;
    written += src_stat.st_size;

    if((error_code = send_fd(&crc16, sizeof(crc16.val), dst_fd, NULL))) goto error_exit;
    error_code = NO_ERROR;
